...
\abstractCZ {
	...
	Provádění vysokorychlostních manévrů při autonomním řízení je problematické bez řádné podpory real-time. Na ČVUT je tým, který se účastní soutěže F1/10 Autonomous Driving Competition s autonomními modely aut. Jejich software pro autonomního řízení je založen na ROS 1, který není vhodný pro real-time aplikace.

	Cílem této práce je migrovat tento software na ROS 2, který byl od základu navržen tak, aby řešil mnoho problémů v této oblasti.

	Dále v práci navrhujeme tracing jako efektivní způsob analýzy běžícího systému na ROS 2 a měření jeho důležitých vlastností.

	Fungování migrovaného softwaru demonstrujeme na modelu vozu F1/10 a v simulátoru Stage. Vyhodnocujeme komunikační latence pomocí rozšířené verze tracing nástrojů pro ROS 2. Dalším výsledkem naší práce je veřejně dostupná sbírka návodů, skriptů a dokumentace, která pokrývá různé aspekty práce s ROS. Tyto návody již pomohly několika lidem.
